...
st.markdown("""
    ...
    /* Premium Full-Height Email Canvas */
    .email-canvas {
...
if not st.session_state["authenticated"]:
    ...
    st.stop()  # blocks everything below until authenticated
...
# Show results dashboard if stored in active memory cache session
if "results" in st.session_state and st.session_state["results"]:
    ...
    for i, r in enumerate(results):
        ...
        with st.expander(f"👤 {r['name']} — {r['company']}"):
            ...
            # Transactional Gateway Controls per item
    ...
    # Combined Export Configuration Tools
    st.markdown("<br>", unsafe_allow_html=True)
    out_df = pd.DataFrame([{
        "name": r["name"],
        "company": r["company"],
